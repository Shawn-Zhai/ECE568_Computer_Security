...
class BioConnect:
	...
	userId			= ''
	authenticatorId		= ''
	# this is where I store verification ID
	stepupId		= ''
	...
	def getAuthenticatorStatus(self):
		...
		if result == False:
			print(headers)
			print(result.content)
			sys.exit("Error: unable to get autenticator status")

		try:
			reply = json.loads(result.content.decode())
		...
		# active status AND at least 1 enrolled
		if (status == "active" and (face_status == "enrolled" or voice_status == "enrolled" or finger_status == "enrolled" or eye_status == "enrolled")):
			return "active"
	...
	def sendStepup(self,
		...
		parameters = {
			...
			# might as well use this for random string
			'transaction_id' : transactionId,
			'message' : message
		}

		# sendingData = json.dumps(parameters)

		result = requests.post(url, data=json.dumps(parameters), headers=headers)

		if result == False:
			print(headers)
			print(result.content)
			sys.exit("Error: unable to get stepUp")
		...
		try:
			...
			print(headers)
			print(result.content)
			sys.exit("Error: unexpected reply for stepUp")

		returnedList = reply.get("user_verification", "")
		self.stepupId = returnedList.get("uuid", "")
	...
	def getStepupStatus(self):
		...
		userVerif = reply.get("user_verification","")
		return (userVerif.get("status", ""))
